						...
						<table class="table">
							<tbody>
								...
								<tr>
									...
									<td>${n.writeId}</td>
									<th>조회수</th>
									<td><fmt:formatNumber type="number" pattern="#,####" value="${n.hit}"/> </td> <!--  ,표 다음이 중요. 4자리씩 끊어서 표현 -->
									<%-- <td><fmt:formatNumber type="number" pattern="#,####원" value="${n.hit}"/> </td> --%> <!-- 숫자 다음에 화폐단위 '원'을 표현할 수 있다. -->
								</tr>
								<tr>
									<th>첨부파일</th>
									<td colspan="3" style="text-align:left; text-indent: 10px;">
										<c:forTokens var="fileName" items="${n.files}" delims="," varStatus="st">
											...
											<a href="${fileName}" style="${style}">${fn:toUpperCase(${fileName})}</a>	<!-- 파일명을 대문자로 표시한다. -->
											<c:if test="${!st.last}">/</c:if>	<!-- 파일 분리 표시자 출력 -->
										</c:forTokens>
									</td>
								</tr>
								<tr class="content">
